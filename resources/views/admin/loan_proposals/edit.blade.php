...
@section('content')
    ...
    <div class="card shadow-md rounded-xl p-6">
        <form id="proposalEditForm" action="{{ route('loan_proposals.update', $loanProposal) }}" method="POST"
            class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="member_id" id="member_select" class="w-full border px-3 py-2 rounded" required>
                    ...
                    @foreach ($members as $member)
                        <option value="{{ $member->id }}" {{ $loanProposal->member_id == $member->id ? 'selected' : '' }}
                            data-member='{{ json_encode($member) }}'>{{ $member->name }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="border-t pt-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="loan_proposal_date" value="{{ $loanProposal->loan_proposal_date }}"
                            class="w-full border px-3 py-2 rounded" required>
                    </div>
                    <div>
                        <label class="font-medium">Type of Business</label>
                        <input type="text" name="business_type" value="{{ $loanProposal->business_type }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="savings_balance" name="savings_balance"
                            value="{{ $loanProposal->savings_balance }}" class="w-full border px-3 py-2 rounded bg-gray-100"
                            step="0.01" readonly required>
                    </div>
                    <div>
                        <label class="font-medium">DPS Balance</label>
                        <input type="number" id="dps_balance" name="dps_balance" value="{{ $loanProposal->dps_balance }}"
            ...
            <div class="border-t pt-4">
                ...
                <div id="previous-loans-container">
                    @if ($loanProposal->previousLoans && $loanProposal->previousLoans->count() > 0)
                        @foreach ($loanProposal->previousLoans as $index => $previousLoan)
                            <div class="previous-loan-entry border rounded p-4 mb-4">
                                <div class="grid grid-cols-6 gap-2 mb-2">
                                    <div>
                                        <label class="font-medium text-sm">Installment No.</label>
                                        <input type="number" name="previous_loans[{{ $index }}][installment_no]"
                                            value="{{ $previousLoan->installment_no }}"
                                            class="w-full border px-2 py-1 rounded text-sm" min="1">
                                    </div>
                                    <div>
                                        <label class="font-medium text-sm">Loan Amount</label>
                                        <input type="number" name="previous_loans[{{ $index }}][loan_amount]"
                                            value="{{ $previousLoan->loan_amount }}"
                                            class="w-full border px-2 py-1 rounded text-sm" step="0.01">
                                    </div>
                                    <div>
                                        <label class="font-medium text-sm">Loan Disbursement Date</label>
                                        <input type="date"
                                            name="previous_loans[{{ $index }}][disbursement_date]"
                                            value="{{ $previousLoan->disbursement_date }}"
                                            class="w-full border px-2 py-1 rounded text-sm">
                                    </div>
                                    <div>
                                        <label class="font-medium text-sm">Repayment Date</label>
                                        <input type="date" name="previous_loans[{{ $index }}][repayment_date]"
                                            value="{{ $previousLoan->repayment_date }}"
                                            class="w-full border px-2 py-1 rounded text-sm">
                                    </div>
                                    <div>
                                        <label class="font-medium text-sm">Probable Repayment Date</label>
                                        <input type="date"
                                            name="previous_loans[{{ $index }}][probable_repayment_date]"
                                            value="{{ $previousLoan->probable_repayment_date }}"
                                            class="w-full border px-2 py-1 rounded text-sm">
                                    </div>
                                    <div>
                                        <label class="font-medium text-sm">Loan Status</label>
                                        <select name="previous_loans[{{ $index }}][loan_status]"
                                            class="w-full border px-2 py-1 rounded text-sm">
                                            <option value="">Select Status</option>
                                            <option value="active"
                                                {{ $previousLoan->loan_status == 'active' ? 'selected' : '' }}>Active
                                            </option>
                                            <option value="completed"
                                                {{ $previousLoan->loan_status == 'completed' ? 'selected' : '' }}>Completed
                                            </option>
                                            <option value="defaulted"
                                                {{ $previousLoan->loan_status == 'defaulted' ? 'selected' : '' }}>Defaulted
                                            </option>
                                        </select>
                                    </div>
                                </div>
                                <button type="button"
                                    ...
                                    style="display: {{ $index > 0 ? 'inline-block' : 'none' }};">Remove</button>
                            </div>
                        @endforeach
                    @else
                        <div class="previous-loan-entry border rounded p-4 mb-4">
            ...
            <div class="border-t pt-4">
                ...
                <div>
                    <input type="number" name="proposed_amount" value="{{ $loanProposal->proposed_amount }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="applicant_signature"
                            value="{{ $loanProposal->applicant_signature }}" class="w-full border px-3 py-2 rounded"
                            placeholder="Signature">
                    </div>
                    <div>
                        <label class="font-medium">Employee's Signature</label>
                        <input type="text" name="employee_signature" value="{{ $loanProposal->employee_signature }}"
            ...
            <div class="border-t pt-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="audited_verified" value="{{ $loanProposal->audited_verified }}"
                            class="w-full border px-3 py-2 rounded" placeholder="Auditor Name">
                    </div>
                    <div>
                        <label class="font-medium">Recommended amount (thousand taka)</label>
                        <input type="number" name="approved_amount_audit"
                            value="{{ $loanProposal->approved_amount_audit }}" class="w-full border px-3 py-2 rounded"
                            step="0.001">
                    </div>
                </div>
                <div class="mt-2">
                    <label class="font-medium">Auditor's Signature</label>
                    <input type="text" name="auditor_signature" value="{{ $loanProposal->auditor_signature }}"
                        class="w-full border px-3 py-2 rounded" placeholder="Signature">
                </div>
            </div>
            ...
            <div class="border-t pt-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="verified_by_manager"
                            value="{{ $loanProposal->verified_by_manager }}" class="w-full border px-3 py-2 rounded"
                            placeholder="Manager Name">
                    </div>
                    <div>
                        <label class="font-medium">Recommended amount (thousand taka)</label>
                        <input type="number" name="approved_amount_manager"
                            value="{{ $loanProposal->approved_amount_manager }}" class="w-full border px-3 py-2 rounded"
                            step="0.001">
                    </div>
                </div>
                <div class="mt-2">
                    <label class="font-medium">Manager's Signature</label>
                    <input type="text" name="manager_signature" value="{{ $loanProposal->manager_signature }}"
                        class="w-full border px-3 py-2 rounded" placeholder="Signature">
                </div>
            </div>
            ...
            <div class="border-t pt-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="verified_by_area_manager"
                            value="{{ $loanProposal->verified_by_area_manager }}" class="w-full border px-3 py-2 rounded"
                            placeholder="Area Manager Name">
                    </div>
                    <div>
                        <label class="font-medium">Approved amount (thousand taka)</label>
                        <input type="number" name="approved_amount_area"
                            value="{{ $loanProposal->approved_amount_area }}" class="w-full border px-3 py-2 rounded"
                            step="0.001">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4 mt-2">
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.getElementById('proposalEditForm').addEventListener('submit', function(e) {
                ...
                let previousLoanIndex = {{ $loanProposal->previousLoans ? $loanProposal->previousLoans->count() : 1 }};
                document.getElementById('add-previous-loan').addEventListener('click', function() {
                    ...
                    inputs.forEach(input => {
                        const name = input.name.replace(/\[(\d+)\]/, '[' + previousLoanIndex + ']');
                        input.name = name;
                        input.value = '';
                    });
